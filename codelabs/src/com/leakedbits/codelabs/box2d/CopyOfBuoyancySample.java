...
public class CopyOfBuoyancySample extends Sample implements ContactListener {
	...
	public void show() {
		...
		/* Create all bodies */
		box = createBox(1f);
		box.setTransform(new Vector2(-1, 2), 0.3f);
		createWalls();
		water = createWater();
	...
	public void applyBuoyancy() {
		for (Tuple<Fixture, Fixture> tuple : wetBodies) {
			...
			CircleShape cA = (CircleShape) fixtureA.getShape();
			...
			for (int i = 0; i < polyA.getVertexCount(); i++) {
				...
				subjectPolygon.add(new Vector2(world.x, world.y)); /*
																	 * IMPORTANTE!
																	 * !
																	 * !!!!!!!!!
																	 * Crear
...
//			List<Vector2> intersectionPoints = PolygonUtils.clipFixtures(
//					subjectPolygon, clipPolygon);
//			for (Vector2 v : intersectionPoints) {
//				Gdx.app.log("Buoyancy", v.x + ", " + v.y);
//			}
//			
...
//						.computeCentroid(intersectionPoints);
...
//									// leading edge
//
//					float dragMag = dragDot * dragMod * edgeLength * density
//							* vel * vel;
...
//					float liftMag = dragDot * liftDot * liftMod * edgeLength
//							* density * vel * vel;
